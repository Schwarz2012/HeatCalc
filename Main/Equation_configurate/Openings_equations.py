...
def openingeq(wallnum, opnum, ceiling, ceilingRS, floor, floorRS, wall, wallRS, wallOp, ctwr, ftwr, wtor, alpha):
    wn = str(wallnum)
    on = str(opnum)
    #Variables seting
    ...
    eq += alpha['Wall ' + wn] * wallOp[wallnum-1][opnum-1].area * (var['Tra'] - var['TwOpi'][wn + on])
    return(eq)
